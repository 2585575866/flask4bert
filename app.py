...
classify_bertWorker = BertWorker(args, args.device_map, classify_graph_path, "CLASS", id2label,"","",None)


msg=[]
classify_bert = BertMsg(msg,True)
...
# next(r)
classify_bertWorker.run_classify(classify_bert,classify_graph_def,True)
...
def predict(text):
    ...
    # print(classify_bert.first)
    classify_bert.first = False
    classify_bert.msg = [text]
    classify_reuslt = classify_bertWorker.run_classify(classify_bert,classify_graph_def,False)

    result={}
    result["classify_result"] = classify_reuslt
